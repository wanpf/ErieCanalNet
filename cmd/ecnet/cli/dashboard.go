...
grafana instance running under the ECNET namespace, and cast a
...
`
const (
	grafanaServiceName = "ecnet-grafana"
	grafanaWebPort     = 3000
)
...
func (d *dashboardCmd) run() error {
	...
	if err != nil {
		return annotateErrorMessageWithEcnetNamespace("Failed to get REST config from Helm %s\n", err)
	}
	...
	if err != nil {
		return annotateErrorMessageWithEcnetNamespace("Failed to get ECNET Grafana service data: %s", err)
	}
	...
	if err != nil {
		return annotateErrorMessageWithEcnetNamespace("Error listing pods: %s", err)
	}
	...
	if grafanaPod == nil {
		return annotateErrorMessageWithEcnetNamespace("No running Grafana pod available")
	}
	...
	if err != nil {
		return annotateErrorMessageWithEcnetNamespace("Error setting up port forwarding: %s", err)
	}
	...
	if err != nil {
		return annotateErrorMessageWithEcnetNamespace("Port forwarding failed: %s", err)
	}
