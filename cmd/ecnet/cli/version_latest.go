...
func getLatestReleaseVersion() (string, error) {
	url := "https://api.github.com/repos/flomesh-io/ErieCanal/releases/latest"
...
func outputLatestReleaseVersion(out io.Writer, latestRelease string, currentRelease string) error {
	...
	if current.LessThan(latest) {
		fmt.Fprintf(out, "\nECNET %s is now available. Please see https://github.com/flomesh-io/ErieCanal/releases/latest.\nWARNING: upgrading could introduce breaking changes. Please review the release notes.\n\n", latestRelease)
	}
	return nil
}
