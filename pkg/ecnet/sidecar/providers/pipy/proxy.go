...
type Proxy struct {
	...
	// The time this Proxy connected to the ECNET control plane
...
func (p *Proxy) StatsHeaders() map[string]string {
	...
	// Prometheus config in the ECNET Helm chart.
	...
	return map[string]string{
		"ecnet-stats-pod":       podName,
		"ecnet-stats-namespace": podNamespace,
		"ecnet-stats-kind":      podControllerKind,
		"ecnet-stats-name":      podControllerName,
	}
}
...
func (p *Proxy) GetCNPrefix() string {
	return sidecar.GetCertCNPrefix(p)
}
...
// NewProxy creates a new instance of a proxy connected to the servers.
func NewProxy(uuid uuid.UUID, svcIdentity identity.ServiceIdentity, ip net.Addr) *Proxy {
	proxyLock.Lock()
	proxyID++
	id := proxyID
