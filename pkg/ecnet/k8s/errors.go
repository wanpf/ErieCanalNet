...
var (
	...
	// errMoreThanOnePodForUUID is an error for when ECNET finds more than one pod for a given xDS certificate. There should always be exactly one Pod for a given xDS certificate.
	errMoreThanOnePodForUUID = fmt.Errorf("found more than one pod for xDS uuid")

	// errDidNotFindPodForUUID is an error for when ECNET cannot not find a pod for the given xDS certificate.
