...
const (
	ecnetCniName       = "ecnet-cni"
	kubeConfigFileName = "ZZZ-ecnet-cni-kubeconfig"
	tokenPath          = "/var/run/secrets/kubernetes.io/serviceaccount/token"

	kubeConfigTemplate = `# Kubeconfig file for ECNET CNI plugin.
apiVersion: v1
kind: Config
clusters:
- name: local
...
contexts:
- name: ecnet-cni-context
  context:
    cluster: local
    user: ecnet-cni
current-context: ecnet-cni-context
`
)
...
func (in *installer) Cleanup() error {
	...
	cniBinPath := path.Join(config.CNIBinDir, ecnetCniName)
...
func writeCNIConfig(ctx context.Context, cniConfig []byte) (string, error) {
	...
	// This section overwrites an existing plugins list entry for ecnet-cni
